...
class MainActivity : Activity() {
    ...
    private var accent="#BEEB00"; private var flashColour="#BEEB00"; private var crestUri:Uri?=null; private var healthSteps=0L
    ...
    private fun colourRow(onPick:(String)->Unit)=LinearLayout(this).apply{orientation=LinearLayout.HORIZONTAL;listOf("#BEEB00","#00E5FF","#FF3B30","#FF9500","#FFFFFF","#9C27B0").forEach{hex->addView(Button(this@MainActivity).apply{text="";background=GradientDrawable().apply{cornerRadius=dp(7).toFloat();setColor(Color.parseColor(hex));setStroke(dp(2),Color.rgb(35,40,35))};setOnClickListener{onPick(hex)}},LinearLayout.LayoutParams(0,dp(54),1f).apply{setMargins(dp(3),0,dp(3),0)})}}
    ...
    override fun onCreate(b:Bundle?){super.onCreate(b);window.statusBarColor=Color.BLACK;window.navigationBarColor=Color.BLACK
        ...
        root.addView(section("✎  WATCH APPEARANCE"){
            ...
            addView(txt("ACCENT COLOUR",13f));addView(colourRow{accent=it})
            addView(txt("CPR FLASH COLOUR",13f).apply{setPadding(0,dp(14),0,0)});addView(colourRow{flashColour=it})
        })
        ...
        root.addView(button("⟳   SYNC EVERYTHING TO WATCH"){syncAll()},LinearLayout.LayoutParams(-1,dp(72)).apply{bottomMargin=dp(12)});root.addView(txt("Samsung Health steps, phone battery, text, accent, CPR flash colour and crest are sent to the watch.",14f,Color.GRAY).apply{gravity=Gravity.CENTER})
