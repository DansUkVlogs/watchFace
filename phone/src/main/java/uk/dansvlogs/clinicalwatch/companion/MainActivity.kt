...
import com.samsung.android.sdk.health.data.request.readDataRequestBuilder
...
class MainActivity : Activity() {
    ...
    private val health by lazy { HealthDataService.getStore(this) }
    ...
    override fun onCreate(b:Bundle?){super.onCreate(b);val root=LinearLayout(this).apply{orientation=LinearLayout.VERTICAL;setPadding(40,40,40,40);setBackgroundColor(Color.rgb(16,19,16))};fun title(s:String,z:Float=18f)=TextView(this).apply{text=s;textSize=z;setTextColor(Color.WHITE);setPadding(0,14,0,8)}
        ...
        Button(this).apply{text="SYNC EVERYTHING TO WATCH";setOnClickListener{syncAll()};root.addView(this)};root.addView(title("Samsung Health steps, phone battery, name, accent and crest are sent to the watch.",14f));setContentView(ScrollView(this).apply{addView(root)});scope.launch{delay(500);refreshIfPermitted()}
    }
    private fun connectHealth()=scope.launch{try{status.text="Connecting to Samsung Health…";val granted=health.getGrantedPermissions(setOf(stepPermission));if(!granted.contains(stepPermission))health.requestPermissions(setOf(stepPermission),this@MainActivity);readTodaySteps()}catch(t:Throwable){status.text="Samsung Health error: ${t.message?:t.javaClass.simpleName}"}}
    ...
    private suspend fun readTodaySteps(){val start=LocalDateTime.now().toLocalDate().atStartOfDay();val end=LocalDateTime.now();val request=readDataRequestBuilder(DataTypes.STEPS).setLocalTimeFilter(LocalTimeFilter.of(start,end)).build();val rows=health.readData(request).dataList;healthSteps=rows.sumOf{it.getValue(DataType.StepsType.TOTAL)?.toLong()?:0L};stepsView.text="Today's steps: %,d".format(healthSteps);status.text="Samsung Health connected";sendLiveData()}
    ...
    private fun send(path:String,bytes:ByteArray){Wearable.getNodeClient(this).connectedNodes.addOnSuccessListener{nodes->nodes.forEach{Wearable.getMessageClient(this).sendMessage(it.id,path,bytes)};status.text=if(nodes.isEmpty())"No connected watch found" else "Synced to watch"}}
