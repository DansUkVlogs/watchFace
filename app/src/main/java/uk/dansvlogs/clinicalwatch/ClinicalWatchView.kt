...
class ClinicalWatchView(context: Context) : View(context) {
    ...
    private val batteryReceiver = object : BroadcastReceiver() { override fun onReceive(c: Context?, i: Intent?) { val l=i?.getIntExtra(BatteryManager.EXTRA_LEVEL,-1)?:-1; val s=i?.getIntExtra(BatteryManager.EXTRA_SCALE,-1)?:-1; if(l>=0&&s>0)watchBattery=l*100/s;invalidate() } }
    ...
    init { context.registerReceiver(batteryReceiver,IntentFilter(Intent.ACTION_BATTERY_CHANGED));ContextCompat.registerReceiver(context,dataReceiver,IntentFilter("uk.dansvlogs.clinicalwatch.DATA_CHANGED"),ContextCompat.RECEIVER_NOT_EXPORTED);load();handler.post(ticker) }
    private fun load(){name=prefs.getString("name","DAN")?:"DAN";try{accent=Color.parseColor(prefs.getString("accent","#BEE600"))}catch(_:Throwable){};phoneBattery=prefs.getString("phoneBattery",null)?.toIntOrNull()?:phoneBattery;steps=prefs.getString("steps",null)?.toLongOrNull()?:steps;customImage=try{prefs.getString("crest",null)?.let{val b=Base64.decode(it,Base64.DEFAULT);BitmapFactory.decodeByteArray(b,0,b.size)}}catch(_:Throwable){null}}
    fun setAmbient(v:Boolean){ambient=v;restart()};fun setExternalSteps(v:Long){steps=v.coerceAtLeast(0);invalidate()};fun startSensors(){};fun stopSensors(){}
    ...
    override fun onTouchEvent(e:MotionEvent):Boolean{if(gestures.onTouchEvent(e))return true;if(e.action==MotionEvent.ACTION_UP&&stopwatchMode&&!ambient){val cx=width/2f;val cy=height/2f;val r=min(width,height)*.495f;if(e.y in (cy-r*.20f)..(cy+r*.24f)){if(e.x<cx-r*.12f)toggleStopwatch()else if(e.x>cx+r*.12f)resetStopwatch();return true}};return true}
    ...
    private fun top(c:Canvas,cx:Float,cy:Float,r:Float){
        // More breathing room from the top edge, with substantially larger branding and date.
        drawImageCircle(c,cx,cy-r*.735f,r*.105f)
        textFit(c,name.uppercase(Locale.getDefault()),cx,cy-r*.535f,r*.112f,white,true,r*.74f)
        if(!stopwatchMode){val now=Date();val date=SimpleDateFormat("EEE dd MMM(MM) yyyy",Locale.getDefault()).format(now).uppercase(Locale.getDefault());textFit(c,date,cx,cy-r*.345f,r*.091f,white,true,r*1.18f)}
    }
    ...
    private fun clockInfo(c:Canvas,cx:Float,cy:Float,r:Float){
        sidePanel(c,cx-r*.54f,cy-r*.045f,r*.255f,r*.215f)
        watchIcon(c,cx-r*.650f,cy-r*.115f,r*.058f);textFit(c,"$watchBattery%",cx-r*.485f,cy-r*.082f,r*.091f,white,true,r*.245f)
        phoneIcon(c,cx-r*.650f,cy+r*.070f,r*.054f);textFit(c,phoneBattery?.let{"$it%"}?:"--",cx-r*.485f,cy+r*.105f,r*.091f,white,true,r*.245f)

        // Two recognisable footprint silhouettes sit above a large xx,xxx-sized step count.
        sidePanel(c,cx+r*.54f,cy-r*.045f,r*.255f,r*.215f)
        feetIcon(c,cx+r*.54f,cy-r*.120f,r*.070f)
        textFit(c,steps?.let{"%,d".format(it)}?:"--",cx+r*.54f,cy+r*.105f,r*.135f,accent,true,r*.455f)

        digitalPanel(c,cx,cy+r*.335f,r)
        textFit(c,SimpleDateFormat("HH:mm:ss",Locale.getDefault()).format(Date()),cx,cy+r*.390f,r*.178f,white,true,r*.875f)
    }
    private fun stopwatchInfo(c:Canvas,cx:Float,cy:Float,r:Float){actionPanel(c,cx-r*.53f,cy-r*.055f,r*.27f,r*.205f);playPauseIcon(c,cx-r*.53f,cy-r*.055f,r*.075f,stopwatchRunning);actionPanel(c,cx+r*.53f,cy-r*.055f,r*.27f,r*.205f);resetIcon(c,cx+r*.53f,cy-r*.055f,r*.075f);digitalPanel(c,cx,cy+r*.335f,r);val ms=stopwatchMs();val total=ms/1000;val tenths=(ms/100)%10;val sec=total%60;val min=(total/60)%60;val hr=total/3600;textFit(c,String.format(Locale.getDefault(),"%02d:%02d:%02d:%d",hr,min,sec,tenths),cx,cy+r*.390f,r*.142f,white,true,r*.875f)}
    private fun digitalPanel(c:Canvas,x:Float,y:Float,r:Float){panel(c,x-r*.47f,y-r*.17f,x+r*.47f,y+r*.17f,r*.045f,true)}
    ...
    private fun feetIcon(c:Canvas,x:Float,y:Float,s:Float){
        paint.style=Paint.Style.FILL;paint.color=accent
        c.save();c.rotate(-18f,x-s*.42f,y);c.drawOval(RectF(x-s*.73f,y-s*.58f,x-s*.23f,y+s*.47f),paint);c.drawCircle(x-s*.39f,y-s*.68f,s*.14f,paint);c.drawCircle(x-s*.57f,y-s*.62f,s*.11f,paint);c.restore()
        c.save();c.rotate(18f,x+s*.42f,y);c.drawOval(RectF(x+s*.23f,y-s*.47f,x+s*.73f,y+s*.58f),paint);c.drawCircle(x+s*.39f,y+s*.68f,s*.14f,paint);c.drawCircle(x+s*.57f,y+s*.62f,s*.11f,paint);c.restore()
    }
    ...
    private fun textFit(c:Canvas,s:String,x:Float,y:Float,maxSize:Float,col:Int,bold:Boolean,maxWidth:Float){paint.shader=null;paint.style=Paint.Style.FILL;paint.typeface=Typeface.create("sans-serif-condensed",if(bold)Typeface.BOLD else Typeface.NORMAL);paint.textAlign=Paint.Align.CENTER;paint.textSize=maxSize;paint.color=col;if(paint.measureText(s)>maxWidth)paint.textSize=maxSize*(maxWidth/paint.measureText(s));c.drawText(s,x,y,paint)}
}
